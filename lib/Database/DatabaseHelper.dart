...
class DatabaseHelper{
  ...
  _initDB() async{
    ...
    String path = join(directory.path,_dbName); // join function is not recognized directly first we import path package and than use it.
    return await openDatabase(path,onCreate:(Database db, int version) async{
      await db.execute('''create table notes(id integer primary key autoincrement,title text,content text,date text, time text)''');
    },version: _dbVersion);
  }
